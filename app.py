        ...
        if st.button("🚀 Lancer le calcul des tarifs", type="primary", use_container_width=True):
            try:
                with st.spinner("Calcul des tarifs en cours..."):
                    ...
                    required_cols_commandes = ["Nom du partenaire", "Service de transport", 
                                            "Pays destination", "Poids expédition"]
                    required_cols_tarifs = ["Partenaire", "Service", "Pays", 
                                          "PoidsMin", "PoidsMax", "Prix"]
                    
                    commandes = st.session_state.data_cache['commandes']
                    tarifs = st.session_state.data_cache['tarifs']
                    
                    missing_cols_commandes = [col for col in required_cols_commandes 
                                            if col not in commandes.columns]
                    missing_cols_tarifs = [col for col in required_cols_tarifs 
