...
def display_results_tab(commandes_tarifées: pd.DataFrame, commandes_sans_tarif: pd.DataFrame):
    ...
    # Boutons de téléchargement avec clés uniques
    st.download_button(
...
def display_weight_control_tab(compiled_file, facture_file):
    ...
    # Bouton de téléchargement pour les écarts de poids avec clé unique
    st.download_button(
...
def display_analysis_tab(commandes_tarifées: pd.DataFrame):
    ...
    col3.metric("Nombre de Commandes", nb_commandes)
...
def main():
    st.title("Application de Calcul des Tarifs et Contrôle des Écarts de Poids")
    
    # Configuration de la taxe par transporteur
    taxe_par_transporteur = {
    ...
    commandes_file = st.file_uploader("Fichier de commandes", type=["xlsx"])
    tarifs_file = st.file_uploader("Fichier de tarifs", type=["xlsx"])
    prix_achat_file = st.file_uploader("Fichier prix d'achat (optionnel)", type=["xlsx"])
    compiled_file = st.file_uploader("Fichier compilé", type=["xlsx"])
    facture_file = st.file_uploader("Fichier de facturation", type=["xlsx"])
    ...
    if commandes_file and tarifs_file:
        try:
            ...
            st.info("Veuillez vérifier les fichiers et réessayer.")
